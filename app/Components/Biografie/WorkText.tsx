...
export default function WorkText() {
    ...
    return (
        <ul className="max-w-max m-auto text-left list-disc">
            {workText.map((work) => (
                <li key="3" className="">
                    <span key="4" className="">{work.name}</span>
                    {work.sub?.map((s) => (
                        <div key="5" className="flex items-center space-x-3 rtl:space-x-reverse">
                            <svg key="6" className="flex-shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512">
