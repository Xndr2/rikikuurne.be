...
export default function Verenigingen() {
    ...
    return (
        <ul className="max-w-max m-auto text-left list-disc text-white">
            {vers.map((ver) => (
                <li key="1" className="">
                    <span className="text-gray-500 dark:text-gray-400">{ver.group}</span>
                    {ver.sub?.map((s) => (
                        <div key={Math.random()} className="flex items-center space-x-3 rtl:space-x-reverse">
                            <svg className="flex-shrink-0 w-4 h-4 invert" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512">
