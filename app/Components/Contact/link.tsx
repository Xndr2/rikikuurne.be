...
export default function Links(props: LinkProps) {
    const { links } = props;
    return (
        <div key="" className="grid grid-cols-1 md:grid-cols-2 gap-4 md:w-max m-auto my-4">
            {links.map((l) => (
                <Link key="" target='_blank' href={l.link} className="flex items-center text-left border-2 rounded-lg p-2 hover:border-blue-300">
                    <Image
                        src={l.icon}
                        width={64}
                        height={64}
